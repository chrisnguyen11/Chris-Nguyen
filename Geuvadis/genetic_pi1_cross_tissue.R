...
predpop_list<-c("ALL","AFA","CAU","CHN","HIS")

# geuvadis pops
obspop_list<-c("ALL","CEU","FIN","GBR","TSI","YRI")

R2_filtering<-c(-1)

columns<-expand.grid(predpop_list,R2_filtering) %>% 
  arrange(Var1,Var2) %>%
...
for (obspop in obspop_list)
  ...
  observed_expression<-fread("zcat /home/chris/geuvadis/" %&% obspop %&% "/expression/" %&% obspop %&% "_age_sex_exam_10PCS_PF_adj_rinv_TOPMED_expression10_peer_factor_adjusted.txt.gz", header = T, sep = '\t',stringsAsFactors = F) %>%
  ...
  for (i in 1:nrow(columns))
    ...
    models<-fread("/home/chris/geuvadis/baseline_elasticnet/imputed_results/" %&% predtiss %&% "/" %&% predtiss %&% "_" %&% predpop %&% "_unfiltered_model_" %&% obspop %&% "_geno_prediction_summary.txt",stringsAsFactors = F)
    names<-fread("/home/chris/geuvadis/baseline_elasticnet/imputed_results/geno_prediction_summary_header.txt",header=F,stringsAsFactors = F)
    names<-c(unlist(names))
    colnames(models)<-names
    models<-models %>% select(gene_id, test_R2_avg)

    predicted_expression<-fread("/home/chris/geuvadis/baseline_elasticnet/imputed_results/" %&% predtiss %&% "/" %&% predtiss %&% "_" %&% predpop %&% "_unfiltered_model_" %&% obspop %&% "_geno_predicted_expression.txt",header=T,stringsAsFactors = F)
...
#        cat("WARNING: possible at higher R2 thresholds, but unlikely at low thresholds unless there is a gene name mismatch. Check that your genenames are the same between files")
      ...
      fwrite(filtered_gene_R2,"/home/chris/geuvadis/baseline_elasticnet/imputed_results/" %&% predtiss %&% "/spearman/" %&% predtiss %&% "_" %&% predpop %&% "_R2_" %&% j %&% "_model_" %&% obstiss %&% "_" %&% obspop %&% "_expression_spearman_correlation.txt",col.names = T,sep='\t')
...
fwrite(pi1_matrix, "/home/chris/geuvadis/baseline_elasticnet/imputed_results/" %&% predtiss %&% "/pi1/" %&% predtiss %&% "models_"%&% obstiss %&%"_pi1_R2-1.txt", sep = '\t', col.names=T,row.names = T)
