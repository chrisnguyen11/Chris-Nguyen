...
def check_arg(args=None):
    ...
    parser.add_argument('--pop','--population', '-p',
			help='population of the VCF file',
			required='True'
			)
...
dict = args.dict
pop = args.pop
#chrfile = chrpath + "chr" + c + ".all.tiss.vcf.gz"
#chrfile = chrpath + pop + ".chr" + c + ".hg38.maf.0.01.vcf.gz"
chrfile = chrpath + pop + ".chr" + c + ".hg38.maf.vcf.gz"

# get dosage file data
if(os.path.exists(outpath + '/dosages/') == False):
    os.mkdir(outpath + '/dosages/')
if (dict == True):
    mapfile=gzip.open(outpath + "/dosages/" + pop + ".cpos_rsid_map_chr" + c + ".txt.gz","wb")
    mapfile.write("cpos\trsid\n")
outdosage = gzip.open(outpath + "/dosages/" + pop + ".chr" + c + ".maf" + str(mafthresh) + ".R2" + str(r2thresh) + ".dosage.txt.gz","wb")
for line in gzip.open(chrfile):
    ...
    if(line.startswith('#CHROM')): #only one line should match #CHROM
        ...
        outsamples = open(outpath + "/dosages/samples.txt","w")
        outsamples.write("\n".join(ids))
        outsamples.close()
        continue
...
#        (altAF,MAF,R2,ER2,typed,AC,AN) = info.split(";")
    elif(re.search('EX_TARGET',info)): #look for 'EX_TARGET' to decide whether to split into "" or ""
	(AC,AN,DP,AF,EAS_AF,EUR_AF,AFR_AF,AMR_AF,SAS_AF,EX_TARGET,VT,NS,MAF) = info.split(";")
	R2="R2=1"
    else:
	(AC,AN,DP,AF,EAS_AF,EUR_AF,AFR_AF,AMR_AF,SAS_AF,VT,NS,MAF) = info.split(";")
	R2="R2=1"
    r2 = float(R2.split("=")[1]) #get IMPUTE2 info score as float
    maf = float(MAF.split("=")[1])
    gt_dosagerow = arr[9:]
#    print(str(gt_dosagerow))
    dosagerow = map(lambda x : float(x.split("|")[0])+float(x.split("|")[1]), gt_dosagerow) #lambda function to split each info entry and collect the dosage
#    print(str(dosagerow))
    ...
    if (cpos == True and r2 > r2thresh and minor > mafthresh):
        ...
        dosages = ' '.join(map(str,dosagerow))
        output = "chr" + chr + ' ' + id + ' ' + pos + ' ' + ref + ' ' + alt + ' ' + str(altAF) + ' ' + dosages + '\n'
        outdosage.write(output)
