...
predpop_list<-c("ALL","AFA","CAU","HIS")

obspop_list<-c("ALL","AFA","CAU","CHN","HIS")
...
for (obspop in obspop_list)
  ...
  observed_expression<-fread("zcat /home/chris/topmed/expression/" %&% obspop %&% "/" %&% obstiss %&% "/expression/" %&% obstiss %&% "_" %&% obspop %&% "_age_sex_exam_10PCS_PF_adj_rinv_TOPMED_expression10_peer_factor_adjusted.txt.gz", header = T, sep = '\t',stringsAsFactors = F) %>%
  ...
  for (i in 1:nrow(columns))
    ...
    models<-fread("/home/chris/topmed/expression/baseline_elasticnet/imputed_results/cross_tissue/" %&% predtiss %&% "_" %&% predpop %&% "_unfiltered_model_" %&% obstiss %&% "_" %&% obspop %&% "_geno_prediction_summary.txt",stringsAsFactors = F)
    ...
    predicted_expression<-fread("/home/chris/topmed/expression/baseline_elasticnet/imputed_results/cross_tissue/" %&% predtiss %&% "_" %&% predpop %&% "_unfiltered_model_" %&% obstiss %&% "_" %&% obspop %&% "_geno_predicted_expression.txt",header=T,stringsAsFactors = F)
...
#        cat("WARNING: possible at higher R2 thresholds, but unlikely at low thresholds unless there is a gene name mismatch. Check that your genenames are the same between files")
      ...
      fwrite(filtered_gene_R2,"/home/chris/topmed/expression/baseline_elasticnet/imputed_results/cross_tissue/" %&% predtiss %&% "_" %&% predpop %&% "_R2_" %&% j %&% "_model_" %&% obstiss %&% "_" %&% obspop %&% "_expression_spearman_correlation.txt",col.names = T,sep='\t')
...
fwrite(pi1_matrix, "/home/chris/topmed/expression/baseline_elasticnet/imputed_results/cross_tissue/pi1/" %&% predtiss %&% "models_"%&% obstiss %&%"_pi1_R2-1.txt", sep = '\t', col.names=T,row.names = T)
